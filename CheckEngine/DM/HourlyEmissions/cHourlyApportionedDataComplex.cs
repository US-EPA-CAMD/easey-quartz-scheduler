...
namespace ECMPS.DM.HourlyEmissions
{
    ...
    public class cHourlyApportionedDataComplex : cHourlyApportionedData
    {
        ...
        public cHourlyApportionedDataComplex(cHourlyRawData hourlyRawData, cFactorFormulae[] factorFormulae, ILogger logger)
                        ...
                        if (rawValueArray[dataPos].HasValue)
                        {
                            ...
                            if (!string.IsNullOrEmpty(equation))
                            {
                                ...
                                else
                                {
                                    ...
                                    _logger.LogError("Formula evaluation failed for unit '{Unit}', location '{Location}', Date {date} and {hour}: {errorResult} [{equation}] ({Method})",
                                         ...
                                         out bool useHiLoadFormulae)
        {
            ...
            else
            {
                ...
                _logger.LogError("Op Time used for formula factors: {0}-{1} ({2})",
        ...
        private bool GetFactorFormulae(DateTime date, int hour, int rawHourOffset, out cFactorFormulae factorFormulae)
        {
            ...
            {
                _logger.LogError("No complex formula covers hour {date}-{hour} ({Method})",
                                 date.ToShortDateString(), hour,
                                 "cHourlyApportionedData.GetDataPosition");
                result = false;
            }
