...
namespace ECMPS.DM.HourlyEmissions
{
    ...
    public abstract class cHourlyApportionedData
    {
        ...
        protected cHourlyApportionedData(cHourlyRawData hourlyRawData, ILogger logger)
        {
            // Set Hourly Raw Data
            ...
            {
                PdemReportIdArray[dataDex] = hourlyRawData.PdemReportId;
        ...
        /// <summary>
        /// The array of PDEM_REPORT_ID
        /// </summary>
        public long[] PdemReportIdArray { get; private set; }
        ...
        public bool Apportion()
        {
            ...
            for (int hourDex = 0; hourDex < HourCount; hourDex++)
            {
                ...
                {
                    _logger.LogError(ex, "Apportion({date}, {hour})", date, hour);
                    result = false;
                }
            }
        ...
        protected int? GetHourOffset(DateTime date, int hour)
        {
            ...
            else if ((hour < 0) || (hour >= 24))
            {
                _logger.LogError("Hour '{hour}' value is not an hour: {date} ({Method})",
                                 hour, date, "cHourlyApportionedData.GetDataPosition");
                result = null;
            }
            else
            {
