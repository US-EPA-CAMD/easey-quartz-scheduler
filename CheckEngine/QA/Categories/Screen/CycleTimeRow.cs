...
{
	public class cCycleTimeRow : cCategory
	{

		#region Private Fields

		private cQAScreenMain mQAScreenMain;
		private DataTable mCycleTimeTable;
		...
		public cCycleTimeRow(cCheckEngine ACheckEngine, cQAScreenMain QAScreenMain, string MonitorLocationId, string TestSumId, DataTable CycleTimeTable)
			: base(ACheckEngine, (cProcess)QAScreenMain, "SCRCYCL", CycleTimeTable)
		{
			InitializeCurrent(MonitorLocationId, TestSumId);

			mQAScreenMain = QAScreenMain;
			mCycleTimeTable = CycleTimeTable;
		...
		{
			SetCheckParameter("Current_Cycle_Time_Test", new DataView(mCycleTimeTable,
			  "", "", DataViewRowState.CurrentRows)[0], eParameterDataType.DataRowView);

			SetCheckParameter("Current_Test", new DataView(mCycleTimeTable,
			  "", "", DataViewRowState.CurrentRows)[0], eParameterDataType.DataRowView);
			...
			SetCheckParameter("Cycle_Time_Test_Records", new DataView(mQAScreenMain.SourceData.Tables["TestSummary"],
			  "test_type_cd = 'CYCLE' and component_id = '" + mComponentID + "'", "", DataViewRowState.CurrentRows), eParameterDataType.DataView);
