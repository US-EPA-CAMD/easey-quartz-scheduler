...
{
  public class cFlowToLoadCheckRow : cCategory
  {

    #region Private Fields

    private cQAScreenMain mQAScreenMain;
    private DataTable mFlowToLoadCheckTable;
    ...
    public cFlowToLoadCheckRow(cCheckEngine ACheckEngine, cQAScreenMain QAScreenMain, string MonitorLocationId, string TestSumId, DataTable FlowToLoadCheckTable)
      : base(ACheckEngine, (cProcess)QAScreenMain, "SCRF2LC", FlowToLoadCheckTable)
    {
      InitializeCurrent(MonitorLocationId, TestSumId);

      mQAScreenMain = QAScreenMain;
      mFlowToLoadCheckTable = FlowToLoadCheckTable;
    ...
    {
        SetCheckParameter("Current_Flow_To_Load_Check", new DataView(mFlowToLoadCheckTable,
        "", "", DataViewRowState.CurrentRows)[0], eParameterDataType.DataRowView);

        SetCheckParameter("Current_Test", new DataView(mFlowToLoadCheckTable,
        "", "", DataViewRowState.CurrentRows)[0], eParameterDataType.DataRowView);

      mSystemID = cDBConvert.ToString(((DataRowView)GetCheckParameter("Current_Flow_To_Load_Check").ParameterValue)["mon_sys_id"]);

      SetCheckParameter("Flow_to_Load_Check_Records", new DataView(mQAScreenMain.SourceData.Tables["QAFlowLoadCheck"],
