                    ...
                    case "LINE":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_LINE " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "HGLME":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_HGLME " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "RATA":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_RATA " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "F2LCHK":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_F2LREF " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "F2LREF":
                        {
                            ...
                            SourceDataTable = new DataTable("FacilityQASuppAttribute");
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM QA_SUPP_ATTRIBUTE " +
                              "LEFT OUTER JOIN VW_QA_SUPP_DATA ON QA_SUPP_ATTRIBUTE.QA_SUPP_DATA_ID = VW_QA_SUPP_DATA.QA_SUPP_DATA_ID " +
                              "where mon_loc_id in (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "7DAY":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_7DAY " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "ONOFF":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_ONOFF " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "CYCLE":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_CYCLE " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "FFACC":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_FFACC " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "FFACCTT":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_FFACCTT " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "FF2LBAS":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_FF2LBAS " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "FF2LTST":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_FF2LTST " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
                    ...
                    case "UNITDEF":
                        {
                            ...
                            SourceDataAdapter = new NpgsqlDataAdapter("SELECT * FROM VW_QA_TEST_SUMMARY_UNITDEF " +
                              "WHERE MON_LOC_ID IN (SELECT MON_LOC_ID FROM TEST_SUMMARY " +
                              "WHERE TEST_SUM_ID = '" + mCheckEngine.TestSumId + "')", mCheckEngine.DbDataConnection.SQLConnection);
                            // this defaults to 30 seconds if we don't override it
                            if (SourceDataAdapter.SelectCommand != null)
