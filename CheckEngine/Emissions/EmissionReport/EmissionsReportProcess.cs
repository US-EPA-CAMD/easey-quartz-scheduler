...
namespace ECMPS.Checks.EmissionsReport
{
    ...
    public class cEmissionsReportProcess : cProcess
    {
        ...
        private bool ExecuteChecksWork_Handle_CheckBandsInit()
        {
            ...
            FCo2cCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FCo2cDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FCo2cMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FCo2cOverallHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FCo2cSubDataMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FCo2mCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FCo2mDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            ComponentAuditCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            DailyCalibrationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            WeeklySystemIntegrityTestCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            WeeklySystemIntegrityTestOperatingDatesCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyEmissionsCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyEmissionsInitializationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyEmissionTestCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyFuelCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FFlowMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FFFQAStatusEvaluationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            //FFuelFlowCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);            
            FFuelFlowCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FFuelFlowInitCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            //FFuelFlowOilCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FH2oCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FH2oDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FH2oMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FHiCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FHiDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            HourlyApportionmentVerificatonCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FHourlyConfigurationEvaluationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FHourlyConfigurationInitializationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            FLmeHourlyCo2mCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLmeHourlyHitCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLmeHourlyNoxmCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLmeHourlySo2mCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLongTermFuelFlowCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FNoxcMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FNoxmCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FNoxmDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FNoxrCalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FNoxrDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FO2DryMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FO2WetMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FO2cSubDataMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FOperatingHourCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            FSo2CalculationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FSo2DerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FSo2MonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FSo2rDerivedHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FSummaryValueEvaluationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FSummaryValueInitializationCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            FLinearityStatusCategoryCO2.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLinearityStatusCategoryNOX.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLinearityStatusCategoryO2D.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLinearityStatusCategoryO2W.InitCheckBands(CheckEngine.DbReplicaConnection);
            FLinearityStatusCategorySO2.InitCheckBands(CheckEngine.DbReplicaConnection);

            FDailyCalibrationStatusCategoryCO2.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyCalibrationStatusCategoryFlow.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyCalibrationStatusCategoryNOx.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyCalibrationStatusCategoryO2Dry.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyCalibrationStatusCategoryO2Wet.InitCheckBands(CheckEngine.DbReplicaConnection);
            FDailyCalibrationStatusCategorySO2.InitCheckBands(CheckEngine.DbReplicaConnection);

            FRATAStatusCategoryCO2O2.InitCheckBands(CheckEngine.DbReplicaConnection);
            FRATAStatusCategoryFlow.InitCheckBands(CheckEngine.DbReplicaConnection);
            FRATAStatusCategoryH2O.InitCheckBands(CheckEngine.DbReplicaConnection);
            FRATAStatusCategoryH2OM.InitCheckBands(CheckEngine.DbReplicaConnection);
            FRATAStatusCategoryNOX.InitCheckBands(CheckEngine.DbReplicaConnection);
            FRATAStatusCategoryNOXC.InitCheckBands(CheckEngine.DbReplicaConnection);
            FRATAStatusCategorySO2.InitCheckBands(CheckEngine.DbReplicaConnection);

            DailyInterferenceStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FlowToLoadStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            HgDailyCalibrationStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            HgLinearityStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            HgRataStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            HgWsiStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            LeakStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            // Added MATS 9/29/14
            MATSMDHGRECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDHFRECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDHCLRECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDSO2RECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDHGRHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDHFRHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDHCLRHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMDSO2RHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMMHGCCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMMHFCCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMMHCLCCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCHGRECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCHFRECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCHCLRECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCSO2RECategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCHGRHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCHFRHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCHCLRHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MATSMCSO2RHCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            /* Sorbent Trap related categories */
            MatsHourlyGasFlowMeterEvalCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSamplingTrainEvalCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSamplingTrainInitCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSamplingTrainSamplingRatioReviewCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSorbentTrapEvalCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSorbentTrapInitCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSorbentTrapHourAndRangeEvalCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSorbentTrapOperatingDaysReviewCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            MatsSorbentTrapOverlapEvalCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            /* Flow Averaging Status Checking Categories */
            FlowAveragingDailyCalibrationStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FlowAveragingDailyInterferenceStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FlowAveragingLeakStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            FlowAveragingStatusTestInitCategory.InitCheckBands(CheckEngine.DbReplicaConnection);

            /* NOXR Unused P-PB Monitor Hourly Evaluation */
            NoxrUnusedPpbMonitorHourlyCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            NoxrUnusedPpbDaileyCalibrationStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            NoxrUnusedPpbLinearityStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            NoxrUnusedPpbRataStatusInitCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
            NoxrUnusedPpbRataStatusCategory.InitCheckBands(CheckEngine.DbReplicaConnection);
        ...
        private bool InitSourceData_CombinedHourly(string monPlanId, int rptPeriodId, ref string errorMessage)
        {
            ...
            string sqlTemplate = "Select * From  camdecmpswks.combined_hourly_value_data('{0}', {1}, '{2}')";
            string sql;
                                              ...
                                              ref string AErrorMessage)
        {
            ...
            const string sqlTemplateDailyCalibration
              = "Select * " +
                "  From  camdecmpswks.daily_calibration_test_period_data('{0}', '{1}', '{2}')" +
            ...
            const string sqlTemplateDailyMiscellaneous
              = "Select * " +
                "  From  camdecmpswks.daily_miscellaneous_test_period_data('{0}', '{1}', '{2}')" +
        ...
        private bool InitSourceData_MonitorOther(string monPlanFilter, string monPlanId, ref string errorMessage)
        {
            ...
            //                  string.Format("Select * From  camdecmpswks.SynchronizationManagement('{0}')", monPlanId),
        ...
        private bool InitSourceData_QaData(string monPlanId, ref string errorMessage)
        {
            ...
            string oocSql = string.Format("Select * From camdecmpswks.on_off_calibration_test_all_data('{0}')" +
